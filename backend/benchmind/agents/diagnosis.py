...
class DiagnosisAgent:
    ...
    def _pin_mismatch(self, findings: list[AgentFinding]) -> Diagnosis:
        ...
        for finding in findings:
            if "pin_mapping" not in finding.tags and "wiring_mapping" not in finding.tags:
                continue
            signal_tags = [tag for tag in finding.tags if tag not in {"pin_mapping", "wiring_mapping"} and not tag.startswith("GPIO")]
        ...
        for signal, values in mappings.items():
            ...
            return Diagnosis(
                primary=Hypothesis(
                    ...
                    unresolved_uncertainty=["A clear powered-off continuity/wiring check is still required."],
                ),
                tests=[DiagnosticTest(
                ...
                proposed_fix=[f"Align the {signal} firmware definition and physical wiring to the same verified GPIO."],
    ...
    def _i2c_mismatch(self, case: EngineeringCase) -> Diagnosis:
        ...
        for item in case.evidence:
            ...
            for value in values:
                target = observed_values if "scan" in item.text.lower() or "found" in item.text.lower() else code_values
        ...
        if code_set and observed_set and code_set.isdisjoint(observed_set):
            ...
            return Diagnosis(
                ...
                tests=[DiagnosticTest(
                    ...
                    procedure=["Run an I2C scanner on the target bus.", "Compare the detected address with the address configured in firmware."],
                ...
                proposed_fix=[f"Configure the driver with the verified bus address: one of {sorted(observed_set)}."],
            )
        return Diagnosis()
        ...
        if code_baud and other_baud and code_baud.isdisjoint(other_baud):
            return Diagnosis(
                ...
                tests=[DiagnosticTest(
                    ...
                    procedure=["Read the firmware Serial.begin value.", "Set the serial monitor to exactly the same baud rate.", "Reset the board and inspect the output."],
    ...
    def _explicit_ground_fault(self, case: EngineeringCase) -> Diagnosis:
        patterns = ("gnd not connected", "ground not connected", "missing common ground", "no common ground")
        for item in case.evidence:
            ...
            if any(pattern in lower for pattern in patterns):
                ...
                return Diagnosis(
                    ...
                    tests=[DiagnosticTest(
                        ...
                        procedure=["Power the system off.", "Verify the controller and peripheral ground pins are electrically connected.", "Restore power only after confirming the wiring."],
                    ...
                    proposed_fix=["Connect the communicating devices to a verified common ground, if electrically appropriate for the system."],
                    safety_considerations=["Do not bridge isolated grounds unless the system design explicitly permits it."],
                )
        return Diagnosis()
    ...
    def _compiler_error(self, findings: list[AgentFinding]) -> Diagnosis:
        ...
        return Diagnosis(
            primary=Hypothesis(
                ...
                unresolved_uncertainty=["The exact corrective change depends on the full error context and surrounding source."],
            ),
            tests=[DiagnosticTest(
                name="Reproduce the first error deterministically",
                procedure=["Run the documented build command in the intended toolchain.", "Fix the earliest actionable error first.", "Rebuild before addressing cascaded errors."],
